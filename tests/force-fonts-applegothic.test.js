...
test('Given 天貓價格以自訂字型解碼，When 產生排除 selector，Then 保留 c-price 原始字型', () => {
  ...
  assert.ok(css.includes('[class~="c-price" i]'));
});
...
test('Given Google Workspace 使用 google-material-icons 圖示字體，When 產生排除 selector，Then 保留 Google Material Icons 字體', () => {
  const environment = createEnvironment();
  environment.run();
  const css = environment.styles[0];

  assert.ok(css.includes('[class~="google-material-icons" i]'));
});

test('Given Google Translate 使用 material-icons-extended 圖示字體，When 產生排除 selector，Then 保留 Material Icons Extended 字體', () => {
  const environment = createEnvironment();
  environment.run();
  const css = environment.styles[0];

  assert.ok(css.includes('[class~="material-icons-extended" i]'));
});

test('Given PVE Ext JS 標題列工具及其偽元素圖示，When 產生排除 selector，Then 整條工具結構保留原始字型', () => {
  const environment = createEnvironment();
  environment.run();
  const css = environment.styles[0];

  assert.ok(css.includes('[class~="x-tool" i]'));
  assert.ok(css.includes('[class~="x-tool-tool-el" i]'));
