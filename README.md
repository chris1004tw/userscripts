...
| 名稱 | 說明 | 版本 | 連&#8288;結 |
| :--- | :--- | :---: | :--- |
| 替換字體為 AppleGothic | 將網頁的一般文字改為 AppleGothic，同時保留圖示、程式碼與天貓價格原本適合的字體。若特定網站顯示異常，可從 Tampermonkey 選單將該網站加入黑名單。 | 0.4.12 | [安&#8288;裝](https://github.com/chris1004tw/userscripts/raw/main/force-fonts-applegothic.user.js) |
...
| `force-fonts-applegothic.user.js` | `buildStyles()` 建立一般字體、天貓 `.c-price` 自訂價格字型排除、Icon 排除、GitHub 程式碼語意 selector、首頁 utility 邊界與 exact textarea 特異性；`createClassTokenSelector()`／`createClassPrefixSelectors()` 搭配完整 `google-symbols`、PVE Ext JS `x-tool` 結構 token 與 `-icon` 邊界，保留天貓價格、Google Maps 與 PVE 標題列偽元素等頁面原始字型；`registerMenuCommands()` 管理網站黑名單；`init()` 只注入 CSS，不掃描 DOM 或攔截 Canvas | `tests/force-fonts-applegothic.test.js` |
