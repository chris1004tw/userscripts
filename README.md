...
| 社群媒體影片音量鎖定 | 鎖定 Facebook、Instagram、Threads、X 既有與動態新增影片的音量，防止被平台覆蓋。<br/>透過 Tampermonkey 選單調整並保存音量與靜音；不覆寫原生媒體 prototype，播放前允許平台暫時靜音，開始播放後再恢復鎖定值。<br/>⚠️ 因平台 UI 更新速度過快，不支援平台內建音量滑桿，請透過 Tampermonkey 選單調整音量。 | 0.1.2 | [Install](https://github.com/chris1004tw/userscripts/raw/main/social-media-volume-fix.user.js) |
...
| `social-media-volume-fix.user.js` | `syncToPage()` 同步影片；`handleMediaStateChange()` 於播放後恢復鎖定值；`processVideoScanQueue()` 逐幀處理動態影片 | `tests/social-media-volume-fix.test.js` |
